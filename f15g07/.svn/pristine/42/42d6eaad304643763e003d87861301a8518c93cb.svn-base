$(function () {
    ...
    $("#admin-keys-dropdown .dropdown-menu li a").click(function () {
        ...
        $.get("admin.php?action=getRestaurant&key=" + $(this).text(), function(data) {
    ...
    $(".nav-tabs li").click(function() {
        var tabIndex = $(".nav-tabs li").index($(this));
        ...
        if (tabIndex < $(".nav-tabs li").length - 1) {
            $(".admin-form-group").each(function(index) {
                if (index === tabIndex) {
                    $(this).show().fadeOut(0).fadeIn("slow");
                } else {
                    $(this).hide();
                }
            });
        } else {
            $(".admin-form-group").each(function() {
                $(this).show().fadeOut(0).fadeIn("slow");
            });
        }
    });
    
    $("#admin-form-new").click(function() {
        $("#admin-keys-dropdown .dropdown-label").text("Choose Restaurant");
    ...
    $("#admin-form input[name=key]").keyup(function() {
        $.get("admin.php?action=checkKey&key=" + $(this).val(), function(data) {
            var data = JSON.parse(data);
